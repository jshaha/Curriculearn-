...
const loadObject = (loader: OBJLoader, url: string): Promise<THREE.Object3D> =>
...
const buildSharedBrainData = async (): Promise<SharedBrainData> => {
  ...
  const sampled = sampleCortexSurface(root, { pointCount: POINT_COUNT_TOTAL })
  const spots = buildBrainSpots(sampled.restPositions, { spotFraction: SPOT_FRACTION })

  return { sampled, spots, source }
}
