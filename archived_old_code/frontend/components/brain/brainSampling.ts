...
const ANCHOR_TARGETS: Record<MetricId, Vec3Tuple> = {
  learning_score: [-0.5, 0.18, 0.22],
  cognitive_load: [0.5, 0.18, 0.22],
  engagement: [-0.36, -0.22, 0.18],
  concept_flow: [0.36, -0.22, 0.18],
...
export interface SampledBrainData {
  ...
  pointCount: number;
  brainRadius: number;
  pointSectionLookup: Array<MetricId | null>;
  anchorCentersBySection: Record<MetricId, Vec3Tuple>;
  focusVectorBySection: Record<MetricId, Vec3Tuple>;
  anchorIndicesBySection: Record<MetricId, number[]>;
...
export const sampleCortexSurface = (
  ...
  const pointSectionLookup: Array<MetricId | null> = Array.from(
    { length: totalPoints },
    () => null
  );

  const anchorIndicesBySection = METRIC_IDS.reduce(
  ...
  const anchorCentersBySection = METRIC_IDS.reduce(
    ...
    {
      learning_score: [0, 0, 0],
      cognitive_load: [0, 0, 0],
      engagement: [0, 0, 0],
      concept_flow: [0, 0, 0],
      retention: [0, 0, 0]
    } as Record<MetricId, Vec3Tuple>
  );

  const focusVectorBySection = METRIC_IDS.reduce(
    (accumulator, sectionId) => {
    ...
    {
      learning_score: [0, 0, 1],
      cognitive_load: [0, 0, 1],
      engagement: [0, 0, 1],
      concept_flow: [0, 0, 1],
      retention: [0, 0, 1]
    } as Record<MetricId, Vec3Tuple>
